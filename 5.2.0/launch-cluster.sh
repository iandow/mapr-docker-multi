...
# overlay /opt/mapr/zkdata in order to workaround a wierd permissions error
sudo rm -rf /tmp/zkdata
mkdir -p /tmp/zkdata:/opt/mapr/zkdata
cldb_cid=$(docker run -d --privileged -h ${CLUSTERNAME}c1 -v /tmp/zkdata:/opt/mapr/zkdata -e "DISKLIST=$cldbdisks" -e "CLUSTERNAME=${CLUSTERNAME}" -e "MEMTOTAL=${MEMTOTAL}" -p 9443:8443 -p 9422:22 docker.io/maprtech/mapr-control-cent67:${MAPRVER})
